...
public class BillController {
    ...
    public ResponseEntity<List<BillDTOs.BillItemResponse>> scanBill(
        ...
        String clientApiKey = payload.get("geminiApiKey"); // optional, from UI
        return ResponseEntity.ok(billService.scanAndAddBillItems(roomCode, base64Image, mimeType, clientApiKey));
    }

    /**
     * Parse raw OCR text with Gemini and return structured items WITHOUT saving.
     * Used by the hybrid Tesseract + AI flow on the frontend.
     */
    @PostMapping("/parse-text")
    public ResponseEntity<List<Map<String, Object>>> parseText(
            @PathVariable String roomCode,
            @RequestBody Map<String, String> payload) {
        String rawText = payload.get("text");
        String clientApiKey = payload.get("geminiApiKey");
        return ResponseEntity.ok(billService.parseTextWithGemini(rawText, clientApiKey));
    }
}
