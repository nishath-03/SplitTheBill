...
public class BillService {
    ...
    /** Names that must never be treated as billable items. */
    private static final java.util.regex.Pattern TOTAL_LINE_PATTERN = java.util.regex.Pattern.compile(
        "(?i)^\\s*(grand\\s+)?total[:\\s].*$" +
        "|(?i)^\\s*sub[\\s\\-]?total[:\\s]?.*$" +
        "|(?i)^\\s*(net|bill|amount|payable|due)[:\\s]?.*$" +
        "|(?i)^\\s*(cgst|sgst|igst|vat|gst|tax|cess)[:\\s].*$" +
        "|(?i)^\\s*service\\s+(charge|fee|tax)[:\\s]?.*$" +
        "|(?i)^\\s*round[\\s\\-]?off[:\\s]?.*$" +
        "|(?i)^\\s*discount[:\\s]?.*$"
    );
    ...
    private static boolean isTotalOrTaxLine(String name) {
        ...
        // Exact single-word matches (case-insensitive)
        if (trimmed.equalsIgnoreCase("total") ||
            trimmed.equalsIgnoreCase("total:") ||
            trimmed.equalsIgnoreCase("subtotal") ||
            trimmed.equalsIgnoreCase("sub-total") ||
            trimmed.equalsIgnoreCase("grand total")) return true;
        return TOTAL_LINE_PATTERN.matcher(trimmed).matches();
    }
    ...
    public List<BillDTOs.BillItemResponse> scanAndAddBillItems(String roomCode, String base64Image, String mimeType, String clientApiKey) {
        ...
        try {
            ...
            for (Map<String, Object> rawItem : itemsList) {
                ...
                // Skip total/tax/subtotal lines — must never become bill items
                if (isTotalOrTaxLine(itemName)) {
                    log.debug("Skipping total/tax line: {}", itemName);
                    continue;
                }
    ...
    public List<Map<String, Object>> parseTextWithGemini(String rawText, String clientApiKey) {
        ...
        String prompt = "You are an expert restaurant receipt parser. The text below was extracted via OCR from a physical receipt.\n" +
            ...
            "Rules:\n" +
            "- Include individual food/drink items only\n" +
            "- Do NOT include CGST, SGST, IGST, VAT, service charge, packaging, subtotal, or grand total lines\n" +
