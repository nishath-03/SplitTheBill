...
**A bill splitter web application.** A host creates a session with a QR code → Friends join via QR scan (no app needed) → Bill items are added live → System calculates each person's share → Friends transfer money to host → Host confirms → Session settles. Includes a **Canvas spinner wheel** for fun (e.g., "Who pays the tip?").
...
*Built with ❤️ for SplitTheBill · Java + Spring Boot + React*
